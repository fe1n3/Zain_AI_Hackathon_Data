...
def generate_financial_transactions(customers, n=2000):
    methods = ["Top-up", "Bill Payment", "Roaming", "Add-on"]
    data = []
    for _ in range(n):
        cust = random.choice(customers)
        data.append({
...
def generate_product_catalog(n=50):
    product_names = ["Unlimited Social", "Roaming Pack", "Family Bundle", "Student Offer"]
    data = []
    for i in range(1, n+1):
        data.append({
            "product_id": i,
...
# -----------------------------
# Challenge 2: Enterprise Resource Planning (ERP) Data
# -----------------------------
...
# -----------------------------
# Challenge 3: Transaction Management Data
# -----------------------------
def generate_customers_finance(n=500):
    data = []
    for i in range(1, n+1):
        data.append({
            "customer_id": f"CF{i:05d}",
            "name": fake.name(),
            "account_type": random.choice(["Savings", "Checking", "Business"]),
            "balance_bhd": round(random.uniform(0, 50000), 2),
            "join_date": fake.date_between(start_date='-10y', end_date='today')
        })
    return pd.DataFrame(data)

def generate_transactions(customers, n=2000):
    ...
    for _ in range(n):
        ...
        data.append({
            ...
            "amount_bhd": round(random.uniform(10, 5000), 2),
...
def main():
    ...
    financial_df = generate_financial_transactions(customers_df.to_dict(orient="records"), 10000)
    save_csv(financial_df, os.path.join(c1_dir, "FinancialTransactions.csv"))
    ...
    # ----- Challenge 2 -----
    c2_dir = os.path.join(base_dir, "EnterpriseResourcePlanning")
    ...
    # ----- Challenge 3 -----
    c3_dir = os.path.join(base_dir, "TransactionManagement")
    ensure_folder(c3_dir)

    finance_customers_df = generate_customers_finance(10000)
    save_csv(finance_customers_df, os.path.join(c3_dir, "Customers.csv"))

    transactions_df = generate_transactions(finance_customers_df.to_dict(orient="records"), 10000)
    save_csv(transactions_df, os.path.join(c3_dir, "Transactions.csv"))
